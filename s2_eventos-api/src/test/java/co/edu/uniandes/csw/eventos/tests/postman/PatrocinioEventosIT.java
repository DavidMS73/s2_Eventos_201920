...
import co.edu.uniandes.csw.eventos.resources.PatrocinioEventosResource;
...
/**
 * Pruebas de integracion del recurso de PatrocinioEventos.
 *
 * @author Germán David Martínez Solano
 */
@RunWith(Arquillian.class)
public class PatrocinioEventosIT {

    private static final String COLLECTION = "PatrocinioEventos.postman_collection";

    @Deployment(testable = true)
    public static WebArchive createDeployment() {
        return ShrinkWrap.create(WebArchive.class, "s2_eventos-api.war")//War del modulo api
                // Se agrega las dependencias
                .addAsLibraries(Maven.resolver().loadPomFromFile("pom.xml")
                        .importRuntimeDependencies().resolve()
                        .withTransitivity().asFile())
                // Se agregan los compilados de los paquetes de servicios
                .addPackage(PatrocinioEventosResource.class.getPackage()) //No importa cual recurso usar, lo importante es agregar el paquet
                .addPackage(PatrocinioDetailDTO.class.getPackage()) //No importa cual dto usar, lo importante es agregar el paquete.
